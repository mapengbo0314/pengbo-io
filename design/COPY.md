...
3. 🖱️ **The test suite is you, clicking around.** No unit tests, no integration tests, and nothing verifying one tenant can't read another tenant's data.
...
6. ✍️ **Schema changes are hand-run.** The database and the code drift apart because migrations aren't versioned or synced with deploys.
...
The engagement is fixed-scope on purpose — no hourly meters, no scope creep. Success means your team can run the whole system without us. And if you want us to stay on after — a retainer, the next phase, new features — that's an option you choose, never a dependency you're stuck with.
